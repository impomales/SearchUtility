...
typedef struct record* RecordPtr;	// holds the file path and token frequency.
struct Node
{
	RecordPtr data;	
	struct Node *next;
...
{
	NodePtr front;
	int (*cf)(void *, void*);	// ptr to comparator function.
	int (*cf2)(void *, void*);	// ptr to comparator2 function.
	void (*df)(void *);			// ptr to destroyer function.
};
